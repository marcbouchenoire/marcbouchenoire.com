...
<svg xmlns="http://www.w3.org/2000/svg" width="${WIDTH}" height="${HEIGHT}" viewBox="0 0 ${WIDTH} ${HEIGHT}" fill="none">
  <title>${escapeHtml(`${song.title} by ${escapeHtml(song.artist)}`)}</title>
  <defs>
  ...
  <g clip-path="url(#cover-mask)">
    <use href="#cover" fill="light-dark(var(--color-gray-100), var(--color-gray-800))" />
    <path
      clip-rule="evenodd" 
      fill-rule="evenodd" 
  ...
  <g transform="translate(${COVER_WIDTH + PADDING}, 0)">
    <g color="light-dark(var(--color-rose-500), var(--color-rose-400))" fill="currentColor" transform="translate(-2, 1)">
      ${
        song.playing
          ? `
      <g>
    ...
    >
      ${escapeHtml(truncate(song.title, LABEL_LENGTH))}
    </text>
    <text
      fill="light-dark(var(--color-gray-500), var(--color-gray-400))"
      transform="translate(0, 72)"
    >
      ${escapeHtml(truncate(song.artist, LABEL_LENGTH))}
    </text>
  </g>
</svg>`,
    {
