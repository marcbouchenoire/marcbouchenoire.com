...
<svg xmlns="http://www.w3.org/2000/svg" width="${WIDTH}" height="${HEIGHT}" viewBox="0 0 ${WIDTH} ${HEIGHT}" fill="none">
  ...
  <defs>
    <rect id="cover" width="${COVER_WIDTH}" height="${COVER_HEIGHT}" rx="${COVER_RADIUS}" />
    <clipPath id="cover-mask">
      <use href="#cover" />
    </clipPath>
  </defs>
  <style>
    ...
    #cover-background { 
      fill: ${theme.colors.gray[dark ? "800" : "100"]}; 
    }
    #cover-placeholder { 
      fill: ${theme.colors.gray[dark ? "600" : "300"]}; 
    }
    #date { 
      color: ${theme.colors.rose[dark ? "400" : "500"]}; 
      font-size: ${theme.fontSize["2xs"]}; 
      text-transform: uppercase; 
      letter-spacing: ${theme.letterSpacing.widest}; 
    }
    #date-icon {
      display: ${song.playing ? "none" : "inline"};
    }
    #date-animated-icon {
      display: ${song.playing ? "inline" : "none"};
    }
    #date, #title { 
      font-weight: ${theme.fontWeight.semibold}; 
    }
    #title { 
      color: ${theme.colors.gray[dark ? "100" : "700"]}; 
    }
    #artist { 
      color: ${theme.colors.gray[dark ? "400" : "500"]}; 
    }
  </style>
  <g clip-path="url(#cover-mask)">
    <use id="cover-background" href="#cover" />
    <path
      ...
      transform="translate(${(COVER_WIDTH / 2 - COVER_PLACEHOLDER_WIDTH / 2) * 0.875}, ${COVER_HEIGHT / 2 - COVER_PLACEHOLDER_HEIGHT / 2})"
    />
    <image href="${cover}" width="${COVER_WIDTH}" height="${COVER_HEIGHT}" preserveAspectRatio="xMidYMid slice" />
    <use href="#cover" stroke="${dark ? "rgb(255 255 255 / 8%)" : "rgb(0 0 0 / 6%)"}" stroke-width="2" />
  </g>
  <g transform="translate(${COVER_WIDTH + PADDING}, 0)">
    <g id="date" fill="currentColor" transform="translate(-2, 1)">
      <path
        id="date-icon"
        fill="currentColor"
        clip-rule="evenodd"
      ...
      />
      <g id="date-animated-icon">
        <rect width="2" height="8" rx="1" x="3" y="6" fill="currentColor">
        ...
        <rect width="2" height="12" rx="1" x="7" y="4" fill="currentColor">
          <animate attributeName="height" calcMode="linear" dur="1s" repeatCount="indefinite" values="12;8;10;8;14;12" />
          <animate attributeName="y" calcMode="linear" dur="1s" repeatCount="indefinite" values="4;6;5;6;3;4" />
        </rect>
        <rect width="2" height="6" rx="1" x="11" y="7" fill="currentColor">
        ...
        <rect width="2" height="8" rx="1" x="15" y="6" fill="currentColor">
      ...
      <text transform="translate(${ICON_SIZE + 4}, 14)">${date}</text>
    </g>
    <text id="title" fill="currentColor" transform="translate(0, 44)">${truncate(song.title, LABEL_LENGTH)}</text>
    <text id="artist" fill="currentColor" transform="translate(0, 72)">${truncate(song.artist, LABEL_LENGTH)}</text>
  </g>
</svg>`,
    {
      headers: {
