...
<svg xmlns="http://www.w3.org/2000/svg" width="${WIDTH}" height="${HEIGHT}" viewBox="0 0 ${WIDTH} ${HEIGHT}" fill="none">
  ...
  <style>
    ...
    #date { 
      color: ${theme.colors.lime[dark ? "400" : "500"]}; 
    ...
    #rating-fill { 
      fill: ${theme.colors.gray[dark ? "300" : "600"]}; 
    }
    #rewatch-icon {
      display: ${film.rewatch ? "inline" : "none"};
      stroke: ${theme.colors.gray[dark ? "500" : "400"]}; 
    }
  </style>
  <g clip-path="url(#poster-mask)">
    <use id="poster-background" href="#poster" />
    <path
  ...
  <g transform="translate(${POSTER_WIDTH + PADDING}, 0)">
    ...
    <text id="title" fill="currentColor" transform="translate(0, 44)">${truncate(film.title, LABEL_LENGTH)}</text>
    <g transform="translate(-2, 57)">
      <g>
        <use id="rating-background" href="#rating" />
        <rect id="rating-fill" width="${RATING_WIDTH * ((film.rating ?? 0) / 5)}" height="${RATING_HEIGHT}" clip-path="url(#rating-mask)" />
      </g>
      <g id="rewatch-icon" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" transform="translate(${RATING_WIDTH + 6}, 2)">
