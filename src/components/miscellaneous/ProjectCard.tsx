...
interface ProjectInfoProps extends ComponentProps<"span"> {
  /**
   * The project's repository.
   */
  repository: string
 ...
 */
async function ProjectStars({ repository: path, ...props }: ProjectInfoProps) {
 ...
 */
async function ProjectDate({ repository: path, ...props }: ProjectInfoProps) {
  const repository = await getRepository(path)
        ...
        <small className="flex items-center gap-4 text-2xs font-semibold uppercase leading-tight tracking-widest">
          <span className="flex items-center">
            ...
            <Suspense fallback={<Skeleton className="h-[1.2em] w-16" />}>
              <ProjectDate repository={repository} />
            </Suspense>
          </span>
          <span className="flex items-center">
            <svg
            ...
            <Suspense fallback={<Skeleton className="h-[1.2em] w-10" />}>
              <ProjectStars repository={repository} />
            </Suspense>
          </span>
        </small>
        <a
